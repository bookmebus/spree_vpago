module Vpago
  module AcledaMiniApp
    module Session
      ...
      # signed via the X-Acleda-Signature header (HMAC-SHA256 of
      # phone+first_name+last_name, keyed by the shared secret, see
      # SessionHashValidator). The signature is verified before we touch the user
      # so an unauthenticated caller can't mint a session for an arbitrary phone
      # number. We then find or create the customer (tagged with the acleda_bank
      # identity), mint a session JWT, and return a session-based miniAppUrl for
      # ACLEDA to open in the WebView. The generic session_id OAuth grant
      # validates the JWT when the WebView later authenticates.
      class Create
        prepend Spree::ServiceModule::Base

        def call(phone: nil, first_name: nil, last_name: nil, hash: nil)
          ...
          return failure(nil, 'invalid signature') unless valid_hash?
        ...
        attr_reader :phone, :first_name, :last_name, :hash

        def valid_hash?
          ::Vpago::AcledaMiniApp::SessionHashValidator.new(
            phone: phone, first_name: first_name, last_name: last_name, hash: hash
          ).call
        end
