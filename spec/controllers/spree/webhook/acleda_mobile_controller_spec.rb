...
RSpec.describe Spree::Webhook::AcledaMobilesController, type: :controller do
  ...
  describe 'POST return' do
    ...
    it 'response success with status 200 when successfully update the payment' do
      ...
      expect(json_response['data']['PaymentTokenId']).to eq payment.number
    end

    it 'response failed with status 403 when signed hash is invalid' do
      payloads = {
        TransactionId: "16de81d4-b5ef-ef59-16de-81d4b5efef59",
        PaymentTokenId: payment.number,
        TxnAmount: 30,
      ...
      expect(json_response['status']['message']).to eq 'Invalid SignedHash'
    end

    it 'response failed with status 203 when cannot find the payment' do
      payloads = {
        TransactionId: "16de81d4-b5ef-ef59-16de-81d4b5efef59",
        PaymentTokenId: 'Invalid',
        TxnAmount: 30,
        SenderName: "Test User",
        SignedHash: "c5b9be690bde7dc8a0abebb1a45c0850359540a4977aecd4cdf13e15a2edfe79",
      }
