...
RSpec.describe Vpago::AcledaV2::PaymentRequestUpdater, type: :model do
  let(:gateway) { create(:acleda_v2_gateway, auto_capture: true) }
  let(:payment_source) { create(:acleda_payment_source, payment_method: gateway) }
  let(:user) { create(:user) }
  let(:order) { OrderWalkthrough.up_to(:payment) }
  let(:payment) { create(:acleda_v2_payment, payment_method: gateway, source: payment_source, order: order, state: 'processing') }
  ...
  describe '#call' do
    context 'when the transaction succeeds' do
      it 'clears the error message and marks the payment as successful' do
        checker = double(:acleda_v2_status_checker, success?: true, failed?: false)
    ...
    context 'when the transaction fails' do
      it 'records the gateway error message and marks the payment as failed' do
        checker = double(:acleda_v2_status_checker, success?: false, failed?: true, error_message: 'DECLINED')
        allow(gateway).to receive(:check_transaction).with(payment).and_return(checker)

        marker = double(:marker)
        expect(::Vpago::PaymentStatusMarker).to receive(:new)
          .with(payment, options.merge(status: false, description: 'DECLINED'))
          .and_return(marker)
        expect(marker).to receive(:call)

        updater.call

        expect(updater.error_message).to eq('DECLINED')
      end
    end

    context 'when the transaction is still pending' do
      it 'does not mark the payment as either success or failure' do
        checker = double(:acleda_v2_status_checker, success?: false, failed?: false)
        allow(gateway).to receive(:check_transaction).with(payment).and_return(checker)
