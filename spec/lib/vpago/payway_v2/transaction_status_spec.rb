...
RSpec.describe Vpago::PaywayV2::TransactionStatus do
  ...
  describe '#call' do
    it 'call request to check_remote_status & save to response' do
      ...
      VCR.use_cassette('payway_v2_check_transaction_status_approved') do
        subject.call
      end
  ...
  describe '#success?' do
    it 'return true when status is PRE-AUTH' do
      VCR.use_cassette('payway_v2_check_transaction_status_pre_auth') { subject.call }
    ...
    it 'return false when status is not PRE-AUTH or APPROVED' do
      VCR.use_cassette('payway_v2_check_transaction_status_pending') { subject.call }
